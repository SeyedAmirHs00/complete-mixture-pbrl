#!/usr/bin/env python3
"""PEBBLE mixture with zero-last init, split Adam/SGD reward optimizer, two-path w_k.

Reward-model optimizers:
  - network (ensemble): Adam, lr = ``reward_lr`` (default 0.0003)
  - trust α: SGD, lr = ``alpha_lr`` (default 0.005)

Loss matches ``fig6_alpha_curve_train_wk.py`` / ``mixture_reward_model_wk_adam_sgd.py``:
reward path is weighted by detached w_k with frozen coef; trust path is
unweighted with frozen reward deltas.
"""
import numpy as np
import torch
...
from reward_model.mixture_reward_model_wk_adam_sgd import MixtureRewardModel
...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.reward_model = MixtureRewardModel(
...
@hydra.main(config_path='config', config_name='train_PEBBLE_mixture_zero_last_wk_adam_sgd', strict=True)
def main(cfg):
    print(cfg)
