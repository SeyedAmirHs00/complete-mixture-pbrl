        ...
        while True:
            ...
            for m, loader in enumerate(loaders):
                ...
                with torch.no_grad():
                    ...
                    expert_coef = alphas_m_tan.abs() / (alphas_tan_abs_sum + 1e-8)
                cur_loss =cur_loss * expert_coef
                margin_loss = 2 * torch.max(torch.tensor(0), 0.1 - self.alphas.abs().sum())
                cur_loss = cur_loss.mean()
                loss += cur_loss + margin_loss
            ...
            self.total_epochs += 1
            alphas_tan_abs = torch_tanh(self.alphas).abs()
            alphas_tan_abs_sum = alphas_tan.abs().sum()
