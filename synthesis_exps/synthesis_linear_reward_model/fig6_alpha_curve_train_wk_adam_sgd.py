"""Train Fig. 6 partial-adversary α curves with detached w_k and split optimizers.

Same experiment as ``fig6_alpha_curve_train_wk.py``, but::

    reward / θ  : Adam(lr=0.005)
    trust α     : SGD(lr=0.05)

Trust-path loss stays unweighted. Reward-path BCE is reweighted by detached w_k.

Examples
--------
  python fig6_alpha_curve_train_wk_adam_sgd.py --seeds 100 --overwrite --plot
  python fig6_alpha_curve_train_wk_adam_sgd.py --reward-model mlp --methods both --overwrite
  python fig6_alpha_curve_plot.py --run_dir results/synthetic_partial_adversary_alpha_curve_wk_adam_sgd
"""
...
from fig6_alpha_curve_train_wk import detached_wk
...
DEFAULT_LR_REWARD = 0.0003
DEFAULT_LR_ALPHA = 0.0005
OPTIMIZER_TAG = "adam_reward_sgd_alpha"


def make_split_optimizers(
    model_params: Iterable[nn.Parameter],
    alpha: nn.Parameter,
    *,
    lr_reward: float = DEFAULT_LR_REWARD,
    lr_alpha: float = DEFAULT_LR_ALPHA,
) -> Tuple[torch.optim.Adam, torch.optim.SGD]:
    opt_reward = torch.optim.Adam(list(model_params), lr=lr_reward)
    opt_alpha = torch.optim.SGD([alpha], lr=lr_alpha)
    return opt_reward, opt_alpha


def _zero_grad(*opts: torch.optim.Optimizer) -> None:
    for opt in opts:
        opt.zero_grad(set_to_none=True)


def _step(*opts: torch.optim.Optimizer) -> None:
    for opt in opts:
        opt.step()
...
def run_linear_with_alpha_history(
    *,
    ...
    """Train shared linear reward with detached w_k; Adam on θ, SGD on α."""
    ...
    opt_reward, opt_alpha = make_split_optimizers(
        [theta], alpha, lr_reward=lr_reward, lr_alpha=lr_alpha
    )
    ...
    for t in range(1, steps + 1):
        _zero_grad(opt_reward, opt_alpha)
...
def train_mlp_seed_with_history(
    *,
    ...
    """One-seed MLP TTP with detached w_k; Adam on net, SGD on α."""
    ...
    opt_reward, opt_alpha = make_split_optimizers(
        net.parameters(), alpha, lr_reward=lr_reward, lr_alpha=lr_alpha
    )
...
def run_mlp_with_alpha_history(
    *,
    ...
    """Train MLP reward with w_k; Adam on net, SGD on α."""
    rng = np.random.default_rng(seed)
    k, T, d = 4, 50, 16
    device = get_device()
...
def run_alpha_curve_experiment(
    *,
    ...
    """Run all (setting, method) jobs with w_k + Adam(θ) / SGD(α)."""
    methods = resolve_methods(methods)
    settings = list(settings) if settings is not None else list(PARTIAL_ADV_SETTINGS)

    print(
        f"[alpha-curve w_k Adam/SGD] reward_model={reward_model} methods={methods} "
        f"seeds={seeds} steps={steps} lr_reward(Adam)={lr_reward} lr_alpha(SGD)={lr_alpha}",
        flush=True,
    )
    if reward_model == "mlp":
    ...
    idx = 0
    opt_kw = dict(lr_reward=lr_reward, lr_alpha=lr_alpha)
    for sname, skw in settings:
        for mname in methods:
            idx += 1
            print(
                f"[alpha-curve w_k Adam/SGD] {sname:12s} {mname:10s} ({reward_model}) ...",
                flush=True,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out_dir",
        default="results/synthetic_partial_adversary_alpha_curve_wk_adam_sgd_mlp",
    ...
    p.add_argument(
        "--lr-reward",
        type=float,
        default=DEFAULT_LR_REWARD,
        help="Adam lr for reward net / theta (default: 0.005).",
    )
    p.add_argument(
        "--lr-alpha",
        type=float,
        default=DEFAULT_LR_ALPHA,
        help="SGD lr for trust α (default: 0.05).",
    )
