...
def plot_heatmaps(
    ...
    """Reference style: RdYlGn + RdBu, plain black digits, combined 2x3."""
    ...
    # Extra column on the right reserved for colorbars (outside the panels)
    gs = GridSpec(
        2,
        4,
        figure=fig,
    ...
    cax_c = fig.add_subplot(gs[0, 3])
    cax_a = fig.add_subplot(gs[1, 3])
    ...
    cbar_a.set_label(r"mean $\bar\alpha_A$", fontsize=9)
    ...
    def row_fig(grids, fname, cmap, vmin, vmax, cbar_label, signed: bool):
        ...
        axs = [f.add_subplot(gs1[0, j]) for j in range(3)]
        cax = f.add_subplot(gs1[0, 3])
        im = None
        for j, name in enumerate(names):
            im = _panel(
                axs[j],
...
def grids_from_table(table: pd.DataFrame, k: int):
    ...
    for _, r in table.iterrows():
        ...
        grids_a[r["variant"]][int(r["n_N"]), int(r["n_A"])] = float(r["mean_adv_trust"])
    ...
    for n_a in range(k + 1):
        for n_n in range(k - n_a + 1):
            ...
            for v in SHARED_BRANCH_VARIANTS:
                ...
                adv_trust = float(np.mean(abar[:, n_r + n_n :])) if n_a > 0 else np.nan
                rows.append(
                    {
                        "n_R": n_r,
                        "n_N": n_n,
