"""Train Fig. 6 partial-adversary α curves with detached w_k and AdamW.

Same experiment as ``fig6_alpha_curve_train_wk.py``, but the optimizer is::

    AdamW([
        {"params": model.parameters(), "lr": 3e-4, "weight_decay": 1e-2},
        {"params": [alpha],           "lr": 1e-4, "weight_decay": 0.0},
    ])

For the linear reward head, ``model.parameters()`` is the shared ``theta`` vector.

Examples
--------
  python fig6_alpha_curve_train_wk_adamw.py --seeds 100 --overwrite --plot
  python fig6_alpha_curve_train_wk_adamw.py --reward-model mlp --methods both --overwrite
  python fig6_alpha_curve_plot.py --run_dir results/synthetic_partial_adversary_alpha_curve_wk_adamw
"""
...
def run_linear_with_alpha_history(
    *,
    ...
    """Train shared linear reward with detached w_k and AdamW."""
...
def train_mlp_seed_with_history(
    *,
    ...
    """One-seed MLP TTP with detached w_k and AdamW."""
...
def run_mlp_with_alpha_history(
    *,
    ...
    """Train MLP reward with w_k + AdamW; return metrics + α history."""
    rng = np.random.default_rng(seed)
    k, T, d = 4, 50, 16
    device = get_device()
...
def run_alpha_curve_experiment(
    *,
    ...
    """Run all (setting, method) jobs with w_k + AdamW."""
    methods = resolve_methods(methods)
    settings = list(settings) if settings is not None else list(PARTIAL_ADV_SETTINGS)

    print(
        f"[alpha-curve w_k AdamW] reward_model={reward_model} methods={methods} "
        f"seeds={seeds} steps={steps} lr_model={lr_model} lr_alpha={lr_alpha} "
        f"wd_model={weight_decay_model}",
        flush=True,
    )
    if reward_model == "mlp":
    ...
    for sname, skw in settings:
        for mname in methods:
            idx += 1
            print(f"[alpha-curve w_k AdamW] {sname:12s} {mname:10s} ({reward_model}) ...", flush=True)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out_dir",
        default="results/synthetic_partial_adversary_alpha_curve_wk_adamw",
    ...
    p.add_argument("--lr-model", type=float, default=3e-4, help="AdamW lr for reward net / theta.")
    p.add_argument("--lr-alpha", type=float, default=1e-4, help="AdamW lr for trust α.")
    p.add_argument(
        "--weight-decay-model",
        type=float,
        default=1e-2,
        help="AdamW weight decay on reward net / theta (α uses 0).",
    )
