...
def run_sigma(out_root: str, seeds: int, steps: int, overwrite: bool) -> None:
        ...
        for t in targets:
            for mname, ccoef in methods:
                ...
                rho, abar, rms0 = run_shared_variant(
                    ...
                    seed=9201 + 37 * idx,
                    target_rms=t,
                    progress_desc=f"sigma {cfg}/rms={t:g}/{mname}",
                )
                row = {
                    "config": cfg,
                    "method": mname,
                ...
                rows.append(row)
                status_print(
                    f"[sigma] {cfg:7s} {mname:10s} rms_target={t:<4g} "
    ...
    for t in targets:
        n = table[
            (table.config == "3R1N") & (table.method == "no_cons") & (table.target_rms == t)
        ].iloc[0]
        a = table[
            (table.config == "3R1A") & (table.method == "no_cons") & (table.target_rms == t)
        ].iloc[0]
        band_rows.append(
            {
                "target_rms": t,
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Fig. 3: init/sigma sweep, no consensus (fig:synthetic-sigma-consensus)"
    )
