...
from reward_model.mixture_reward_model_ablation import MixtureRewardModel
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                self.agent.update_state_ent(
                    self.replay_buffer, self.logger, self.step, gradient_update=1, K=self.cfg.topK
                )
...
@hydra.main(config_path="config", config_name="train_PEBBLE_mixture_ablation", strict=True)
def main(cfg):
    print(cfg)
