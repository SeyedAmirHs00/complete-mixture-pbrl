...
def run_branch(out_root: str, seeds: int, steps: int, overwrite: bool) -> None:
    ...
    rows = []
    alpha_store: Dict[str, Dict[str, np.ndarray]] = {c: {} for c in order}
            ...
            rows.append(
                {
                    ...
                    "mean_abar_R": float(abar[:, :3].mean()) if cfg != "1R3A" else float(abar[:, 0].mean()),
                    "mean_abar_N": float(abar[:, 3].mean()) if "N" in cfg else np.nan,
                    "mean_abar_A": float(abar[:, -1].mean()) if "A" in cfg else np.nan,
                }
            )
            alpha_store[cfg][v.name] = abar
            print(
    ...
    for cfg in order:
        ...
        ax.set_title(f"{cfg} (shared head)")
...
def run_ablation(out_root: str, seeds: int, steps: int, overwrite: bool) -> None:
    ...
    ensure_dir(out, overwrite)
    betas = (1.0, 1.0, 1.0, 0.0, -1.0)  # 3R1N1A
    variants = [
        ...
        SharedVariant("attach_w", "Attached $w_k$", 0.0, 0.0, use_tanh=True, use_maxnorm=True, use_confidence_weights=True, detach_weights=False),
    ]
    rows = []
        ...
        rows.append(
            {
                ...
                "abar_A": float(abar[:, 4].mean()),
            }
        )
        print(
            f"[ablation] {v.name:10s} correct={rows[-1]['correct']:.3f} "
            f"|corr|={rows[-1]['mean_abs_corr']:.3f} "
            f"aR={rows[-1]['abar_R']:+.2f} aN={rows[-1]['abar_N']:+.2f} aA={rows[-1]['abar_A']:+.2f}"
        )
    pd.DataFrame(rows).to_csv(os.path.join(out, "enhancement_ablation_3R1N1A.csv"), index=False)
