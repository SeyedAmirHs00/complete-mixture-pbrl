    ...
    *,
    ...
    for _ in range(steps):
        ...
        for e in range(k):
            ...
            loss_R = loss_R + (w[:, e : e + 1] * bce).mean() / k
            if not fix_alpha:
                logits_A = coef[:, e : e + 1] * delta.detach()
                loss_A = loss_A + F.binary_cross_entropy_with_logits(logits_A, y[:, e], reduction="mean") / k
...
def train_ds_sym(
    ...
    *,
    ...
    for _ in range(steps):
        ...
        R = (R - R.mean(1, keepdim=True)) / (R.std(1, keepdim=True) + 1e-12)
        ...
        for e in range(k):
            ...
            loss = loss + F.binary_cross_entropy(p_y, y[:, e], reduction="mean") / k
        loss.backward()
        with torch.no_grad():
            if theta.grad is not None:
...
def main() -> None:
    ...
    for qi, q in enumerate(qs):
        ...
        for method in methods:
            ...
            rows.append(row)
            print(
                f"q={q:<4g} {method:8s} global_med={row['global_med']:.3f} "
                f"local_med={row['local_med']:.3f} correct={row['correct']:.3f}"
            )
